...
fn test_storage_backend_with_file_io() {
    ...
    let offset = topic_log.write().append(record).unwrap();

    assert_eq!(offset, 0);
    assert_eq!(topic_log.read().len(), 1);
...
fn test_consumer_group_with_file_io() {
    ...
    // Test initial state
    assert_eq!(consumer_group.read().get_offset("test_topic"), 0);

    // Test setting offset
    consumer_group
        .write()
        .set_offset("test_topic".to_string(), 42);
    assert_eq!(consumer_group.read().get_offset("test_topic"), 42);

    // Test group ID
    assert_eq!(consumer_group.read().group_id(), "test_group");
}
