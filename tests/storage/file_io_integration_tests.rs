...
fn test_std_file_io_append_and_read() {
    ...
    let mut file_handle = StdFileIO::create_with_append_and_read_permissions(&test_file_path).unwrap();
    ...
    assert_eq!(StdFileIO::get_file_size(&file_handle).unwrap(), test_data.len() as u64);
...
fn test_std_file_io_write_truncate_mode() {
    ...
    let mut file_handle = StdFileIO::create_with_write_truncate_permissions(&test_file_path).unwrap();
...
mod io_uring_tests {
    ...
    fn skip_if_io_uring_unavailable() {
        if !IoUringFileIO::is_available() {
            println!("Skipping io_uring tests - not available on this system");
            return;
        }
    }
    ...
    fn test_io_uring_file_io_append_and_read() {
        ...
        let mut file_handle = IoUringFileIO::create_with_append_and_read_permissions(&test_file_path).unwrap();
        
        let test_data = b"Hello, io_uring I/O!";
        let start_position = IoUringFileIO::append_data_to_end(&mut file_handle, test_data).unwrap();
        ...
        assert_eq!(start_position, 0, "First write should start at position 0");
        assert_eq!(IoUringFileIO::get_file_size(&file_handle).unwrap(), test_data.len() as u64);
        assert_eq!(&read_buffer, test_data);
    }
    ...
    fn test_io_uring_write_truncate_mode() {
        ...
        let mut file_handle = IoUringFileIO::create_with_write_truncate_permissions(&test_file_path).unwrap();
    ...
    fn test_io_uring_large_write() {
        ...
        let start_position = IoUringFileIO::append_data_to_end(&mut file_handle, &large_data).unwrap();
        IoUringFileIO::synchronize_to_disk(&mut file_handle).unwrap();
        
        assert_eq!(start_position, 0);
        assert_eq!(IoUringFileIO::get_file_size(&file_handle).unwrap(), large_data.len() as u64);
    }
}
...
fn test_file_io_compatibility() {
    ...
    #[cfg(target_os = "linux")]
    {
        if IoUringFileIO::is_available() {
            ...
            let mut io_uring_handle = IoUringFileIO::create_with_append_and_read_permissions(&io_uring_path).unwrap();
