...
fn test_append_and_read_operations() {
    ...
    let mut file_handle = AsyncFileHandle::create_with_append_and_read_permissions(
        &test_file_path,
        FileIoMode::IoUring,
    )
    .unwrap();
    ...
    assert_eq!(
        file_handle.get_current_file_size_in_bytes().unwrap(),
...
fn test_read_only_mode() {
    ...
    let mut file_handle =
        AsyncFileHandle::open_with_read_only_permissions(&test_file_path, FileIoMode::IoUring)
            .unwrap();
...
fn test_multiple_append_operations() {
    ...
    assert_eq!(
        ...
        "Append operations should return correct start positions"
    );
    assert_eq!(
        file_handle.get_current_file_size_in_bytes().unwrap(),
        total_size as u64
