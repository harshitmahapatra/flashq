...
fn memory_append_read_batches_boundaries() {
    ...
    let topic = "batch_mem_test".to_string();
    ...
    for &n in &[127usize, 128, 129, 300] {
        ...
        for (i, r) in out.iter().enumerate() {
            ...
            assert_eq!(r.record.key.as_deref(), Some(&format!("k{i}")));
        }

        // partial read across boundary
        let mid = n / 3; let len = (n - mid).min(200);
        let part = queue.poll_records_from_offset(&topic, mid as u64, Some(len)).unwrap();
        assert_eq!(part.len(), len);
        for (i, r) in part.iter().enumerate() {
            let idx = mid + i;
            assert_eq!(r.offset as usize, idx);
            assert_eq!(r.record.key.as_deref(), Some(&format!("k{idx}")));
        }
    }
}
