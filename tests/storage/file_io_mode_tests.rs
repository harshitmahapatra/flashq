...
use flashq::storage::r#trait::TopicLog;
...
fn test_storage_backend_with_io_uring() {
    ...
    let record = flashq::Record::new(Some("io_uring_key".to_string()), "io_uring_value".to_string(), None);
    let offset = topic_log.append(record).unwrap();
...
fn test_io_mode_behavior_compatibility() {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if IoUringFileIO::is_available() {
            ...
            assert_eq!(std_topic_log.next_offset(), io_uring_topic_log.next_offset());
